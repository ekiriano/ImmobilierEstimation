...
module.exports = function validateDefaultAppartmentInput(data) {
  ...
  if(!data.rue.match(/\d{2}\s+rue+\s\w+/)){
 ...
 if(data.nombre_etage_total<0){
    errors.nombre_etage_total="nombre_etage_total doit etre un chiffre positive";
  }
  ...
  if(!data.qualite_appartement.match(/[^0-9]/)){
    errors.qualite_appartement="qualite_appartement ne correspond pas au bon format!";
  }
